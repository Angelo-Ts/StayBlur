(() => {
  ...
  const INDICATOR_ID = 'pb-selection-indicator';
  ...
  function frameDescriptor(frame) { const tag = (frame?.tagName || 'iframe').toLowerCase(); const id = typeof frame?.id === 'string' ? frame.id.trim() : ''; const name = typeof frame?.name === 'string' ? frame.name.trim() : ''; const parent = frame?.parentElement; const siblings = parent ? [...parent.children].filter(el => el.tagName === frame.tagName) : []; const siblingIndex = siblings.indexOf(frame); if (id) return `${tag}#${id}`; if (name) return `${tag}[name=${name}]`; return `${tag}[n=${siblingIndex >= 0 ? siblingIndex : '?'}]`; }
  function frameContextKey() { const chain = []; try { let currentWindow = window; while (currentWindow !== currentWindow.top) { const frame = currentWindow.frameElement; if (!frame || frame.nodeType !== 1) return null; chain.unshift(frameDescriptor(frame)); currentWindow = currentWindow.parent; } } catch (_) { return null; } return chain.length ? `frame:${chain.map((part, depth) => `${depth}:${part}`).join('/')}` : 'top'; }
  function ruleAppliesToCurrentFrame(rule) { const current = frameContextKey(); if (!current) return false; return rule.frameKey ? rule.frameKey === current : current === 'top'; }
  let settingsCache = { extensionEnabled: true, selectionEffect: 'blur', selectionIntensity: 60 }, settingsReady = false, rulesCache = null, rulesCacheContext = '', rulesLoad = null;
  async function getSettings() { if (settingsReady) return settingsCache; const r = await chrome.storage.local.get({ [SETTINGS_KEY]: { extensionEnabled: true, selectionEffect: 'blur', selectionIntensity: 60 } }); settingsCache = { extensionEnabled: true, selectionEffect: 'blur', selectionIntensity: 60, ...(r[SETTINGS_KEY] || {}) }; settingsReady = true; return settingsCache; }
  async function getRule(id) { const r = await chrome.storage.local.get({ [keyRule(id)]: undefined }); return r[keyRule(id)]; }
  async function getRules() { const ck = contextKey(); if (rulesCache && rulesCacheContext === ck) return rulesCache; if (rulesLoad && rulesLoad.context === ck) return rulesLoad.promise; const promise = (async () => { const c = context(); const r = await chrome.storage.local.get({ [keyDomain(c.domain)]: [], [keyPage(c.domain, c.path)]: [] }); const ids = [...new Set([...(r[keyDomain(c.domain)] || []), ...(r[keyPage(c.domain, c.path)] || [])])]; if (!ids.length) return []; const loaded = await chrome.storage.local.get(ids.map(keyRule)); return ids.map(id => loaded[keyRule(id)]).filter(Boolean).filter(ruleAppliesToCurrentFrame); })(); rulesLoad = { context: ck, promise }; const result = await promise; if (rulesLoad?.promise === promise) rulesLoad = null; if (contextKey() === ck) { rulesCache = result; rulesCacheContext = ck; } return result; }
  function invalidateRulesCache() { rulesCache = null; rulesCacheContext = ''; rulesLoad = null; }
  async function saveRule(rule) { const dKey = keyDomain(rule.domain), pKey = keyPage(rule.domain, rule.path || '/'); const old = await chrome.storage.local.get({ [dKey]: [], [pKey]: [] }); await chrome.storage.local.set({ [keyRule(rule.ruleId)]: rule, [dKey]: [...new Set([...(old[dKey] || []), rule.ruleId])], [pKey]: [...new Set([...(old[pKey] || []), rule.ruleId])] }); if (contextKey() === `${rule.domain}|${rule.path || '/'}`) { const current = rulesCache || []; rulesCache = [...current.filter(r => r.ruleId !== rule.ruleId), rule]; rulesCacheContext = contextKey(); } return rule; }
  async function updateRule(rule) { const updated = { ...rule, updatedAt: new Date().toISOString() }; await chrome.storage.local.set({ [keyRule(updated.ruleId)]: updated }); if (rulesCache && rulesCacheContext === contextKey()) rulesCache = rulesCache.map(r => r.ruleId === updated.ruleId ? updated : r); return updated; }
  async function deleteRule(id) { const rule = await getRule(id); if (!rule) return; await chrome.storage.local.remove(keyRule(id)); const dKey = keyDomain(rule.domain), pKey = keyPage(rule.domain, rule.path || '/'); const old = await chrome.storage.local.get({ [dKey]: [], [pKey]: [] }); await chrome.storage.local.set({ [dKey]: (old[dKey] || []).filter(x => x !== id), [pKey]: (old[pKey] || []).filter(x => x !== id) }); if (rulesCache) rulesCache = rulesCache.filter(r => r.ruleId !== id); }
  ...
  function effectIsIntact(el, rule) { if (!(el instanceof Element) || !el.isConnected) return false; return el.getAttribute(ATTR) === rule.ruleId && el.classList.contains('pb-effect-base') && el.classList.contains(`pb-effect-${rule.effect || 'blur'}`); }
