(() => {
  ...
  const CONTENT_SCRIPTS=['content/selection-style.js','content/content-script.js','content/focus-overlay.js','content/shadow-style.js'];
  ...
  async function sendToContent(tabId,message){
    try {
      ...
      try {
        ...
        throw new Error(pageBlockedMessage(String(injectError?.message||firstError?.message||injectError)));
      }
    }
  }
  ...
  async function setExtensionEnabled(enabled){
    const stored=await chrome.storage.local.get({[SETTINGS_KEY]:{}}),current=stored[SETTINGS_KEY]||{};
    await chrome.storage.local.set({[SETTINGS_KEY]:{...current,extensionEnabled:Boolean(enabled)}});
  }

  async function getRule(ruleId){
    const key=`rule:${ruleId}`,stored=await chrome.storage.local.get({[key]:null});
  ...
  async function setAllRulesEnabled(enabled){
    const state=await chrome.storage.local.get(null),rules=Object.entries(state).filter(([key,value])=>key.startsWith('rule:')&&value&&typeof value==='object');
    if(!rules.length)return[];
    const now=new Date().toISOString(),updates={};
    for(const[key,rule]of rules)updates[key]={...rule,enabled:Boolean(enabled),status:enabled?'pending':'disabled',updatedAt:now};
    await chrome.storage.local.set(updates);
    return rules.map(([,rule])=>rule.ruleId).filter(Boolean);
  }

  async function deleteRule(ruleId){
    const key=`rule:${ruleId}`,stored=await chrome.storage.local.get({[key]:null}),rule=stored[key];
    if(!rule)return;
    const domainKey=`idx:domain:${rule.domain}`,pageKey=`idx:page:${rule.domain}:${rule.path||'/'}`;
  ...
  async function deleteAllRules(){
    const state=await chrome.storage.local.get(null),keys=Object.keys(state).filter(key=>key.startsWith('rule:')||key.startsWith('idx:'));
  ...
  chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
    (async()=>{
      ...
      if(message.type==='POPUP_DISABLE_ALL_RULES'||message.type==='POPUP_ENABLE_ALL_RULES'){
        const enabled=message.type==='POPUP_ENABLE_ALL_RULES',ruleIds=await setAllRulesEnabled(enabled),tab=sender.tab||await activeTab();
        if(tab?.id){
          try{
            if(enabled)await Promise.all(ruleIds.map(ruleId=>sendToContent(tab.id,{type:'BG_RETRY_RULE_ON_PAGE',ruleId})));
            else await sendToContent(tab.id,{type:'BG_REMOVE_ALL_EFFECTS_PAGE'});
          }catch(_){}
        }
        return sendResponse({ok:true,count:ruleIds.length});
      }
      if(message.type==='POPUP_DISABLE_RULE'||message.type==='POPUP_ENABLE_RULE'){
        const enabled=message.type==='POPUP_ENABLE_RULE',updated=await setRuleEnabled(message.ruleId,enabled);
        if(!updated)return sendResponse({ok:false,error:'rule-not-found'});
        const tab=sender.tab||await activeTab();
        if(tab?.id){
          try{await sendToContent(tab.id,enabled?{type:'BG_RETRY_RULE_ON_PAGE',ruleId:message.ruleId}:{type:'BG_REMOVE_RULE_EFFECT_PAGE',ruleId:message.ruleId});}catch(_){}
        }
        return sendResponse({ok:true,rule:updated});
      }
      if(message.type==='POPUP_DELETE_RULE'){
        const tab=sender.tab||await activeTab();
        if(tab?.id){try{await sendToContent(tab.id,{type:'BG_REMOVE_RULE_EFFECT_PAGE',ruleId:message.ruleId});}catch(_){}
        }
      ...
      if(message.type==='POPUP_DELETE_ALL_RULES'){
        const tab=sender.tab||await activeTab();
        if(tab?.id){try{await sendToContent(tab.id,{type:'BG_REMOVE_ALL_EFFECTS_PAGE'});}catch(_){}
        }
      ...
      const tab=sender.tab||await activeTab();
      if(!tab?.id)return sendResponse({ok:false,error:'no-active-tab'});
      const type=map[message.type]||message.type,response=await sendToContent(tab.id,{...message,type});
