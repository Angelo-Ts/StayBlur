(() => {
  ...
  async function pingContent(tabId) {
    try {
      return await chrome.tabs.sendMessage(tabId, { type: 'CONTENT_GET_STATE' });
    } catch (_) {
      return null;
    }
  }
  ...
  async function cleanupDom(tabId, ruleId = null) {
    ...
    try {
        ...
        func: id => {
          ...
          const remove = element => {
            ...
            element.classList.remove(
              'pb-effect-base',
              ...effects.map(effect => `pb-effect-${effect}`)
            );
          ...
          const selector = id
            ? `[data-progettoblur-rule-id="${CSS.escape(String(id))}"]`
            : '[data-progettoblur-rule-id]';

          for (const element of document.querySelectorAll(selector)) remove(element);

          // Catch orphaned classes left by a runtime that lost its in-memory map.
      ...
      // Cleanup is best-effort on restricted pages. The normal content-script
      // path is still attempted by the caller.
    }
  }
  ...
  async function setExtensionEnabled(enabled) {
    ...
    await chrome.storage.local.set({
      [SETTINGS_KEY]: { ...current, extensionEnabled: Boolean(enabled) }
    });
  }
  ...
  async function setAllRulesEnabled(enabled) {
    ...
    const rules = Object.entries(state).filter(
      ([key, value]) => key.startsWith('rule:') && value && typeof value === 'object'
    );
  ...
  async function syncPageAfterExtensionToggle(tabId, enabled) {
    ...
    const ruleIds = await getEnabledRuleIds();
    for (const ruleId of ruleIds) {
      try {
        await sendToContent(tabId, { type: 'BG_RETRY_RULE_ON_PAGE', ruleId });
      } catch (_) {}
    }
  }
  ...
  async function deleteAllRules() {
    ...
    const keys = Object.keys(state).filter(
      key => key.startsWith('rule:') || key.startsWith('idx:')
    );
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    (async () => {
      ...
      if (message.type === 'POPUP_DISABLE_ALL_RULES' || message.type === 'POPUP_ENABLE_ALL_RULES') {
        ...
        if (tab?.id) {
          try {
            if (enabled) {
              for (const ruleId of ruleIds) {
                await sendToContent(tab.id, { type: 'BG_RETRY_RULE_ON_PAGE', ruleId });
              }
            } else {
      ...
      if (message.type === 'POPUP_DELETE_RULE') {
        ...
        if (tab?.id) {
          try {
            await sendToContent(tab.id, { type: 'BG_REMOVE_RULE_EFFECT_PAGE', ruleId: message.ruleId });
          } catch (_) {}
